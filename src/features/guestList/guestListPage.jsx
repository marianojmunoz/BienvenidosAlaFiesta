...
const GuestListPage = () => {
  ...
  const handleEditChange = (e, id, field) => {
    ...
    if ((field === 'table' || field === 'hasChildren' || field === 'adults') && value && parseInt(value, 10) < 0) {
      value = '0';
    }
    if ((field === 'adults') && value && parseInt(value, 10) < 1) {
      value = '1';
    }
    ...
    if (isEditing) {
      return (
        <TextField
          ...
          autoFocus
          fullWidth
          type={field === 'qty' || field === 'table' || field === 'hasChildren' || field === 'adults' ? 'number' : 'text'}
        />
      );
    }
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, '@media print': { padding: '20px' } }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <ExportButton
              ...
              summary={{
                'Total Adultos:': totalAdults,
                'Total Niños:': totalChildren,
                'Total:': totalGuests,
              }}
              sx={{ '@media print': { display: 'none' } }}
            />
